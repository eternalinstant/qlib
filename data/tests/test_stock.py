...
class TestCodeConversion:
    ...
    def test_tushare_from_tushare(self):
        from data.stock import to_tushare
    ...
    def test_qlib_from_tushare(self):
        from data.stock import to_qlib
    ...
    def test_internal_from_tushare(self):
        from data.stock import to_internal
    ...
    def test_bj_exchange(self):
        from data.stock import to_tushare, to_qlib, to_internal
...
class TestStockInfo:
    ...
    def _patch_load(self):
        return patch("data.stock._load_stock_basic", return_value=MOCK_DF)

    def test_get_name(self):
        from data.stock import get_name
    ...
    def test_get_industry(self):
        from data.stock import get_industry
        with self._patch_load():
            assert get_industry("300750.SZ") == "电池"

    def test_search_by_name(self):
        from data.stock import search
        with self._patch_load():
            results = search("银行")
...
class TestDisplay:
    ...
    def test_display(self):
        from data.stock import display
        with self._patch_load():
    ...
    def test_display_list(self):
        from data.stock import display_list
        with self._patch_load():
...
class TestGetAllCodes:
    ...
    def test_all_tushare(self):
        from data.stock import get_all_codes
        with self._patch_load():

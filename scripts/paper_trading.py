...
CONFIG_PATH = PROJECT_ROOT / "src" / "strategy" / "configs" / "models" / "qvf_alpha158_core12.yaml"
...
def generate_signals() -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        # Get the latest date available in the frame
        dates_in_frame = frame.index.get_level_values("datetime").unique()
        latest_rb = dates_in_frame.max()

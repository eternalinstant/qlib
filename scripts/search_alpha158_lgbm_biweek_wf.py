#!/usr/bin/env python3
"""Walk-forward search for Alpha158 + LightGBM biweekly rebalancing strategies.

Key differences from the fixed-train search (search_alpha158_lgbm_biweek.py):
1. Rolling training windows (3yr train -> 1yr test -> roll forward)
2. OOS annual return is a hard constraint AND penalty term in rank_score
3. Relaxed targets: full-sample >18%, OOS >12%, max_dd <10%
4. Walk-forward OOS is computed on test-period returns only (no look-ahead)

The search is intentionally bounded:
- Beam search over factor subsets with <8 factors
- Stage 1: fixed params, Stage 2: grid tuning on top combos
- All evaluations include commissions, slippage, impact, and tradability constraints.
"""
...
from strategy.scorer_lgbm import walk_forward_train_predict
...
OUTPUT_COLUMNS = [
    ...
    "sharpe_ratio",
    "total_return",
    # OOS metrics (walk-forward test periods concatenated)
    "oos_annual",
    "oos_max_dd",
    "oos_sharpe",
    # Train metrics (in-sample, first training window)
    "train_annual",
    "train_max_dd",
    "train_sharpe",
    # OOS-only (strict, only test period dates)
    "wf_oos_annual",
    "wf_oos_max_dd",
    "wf_oos_sharpe",
    # Pass / score
...
def rank_score_wf(row: dict, args) -> float:
    """Walk-forward rank score: OOS annual is a penalty, not just a bonus.

    - Base: full-sample annual return
    - Penalty: drawdown excess over target
    - Penalty: small factor bonus
    - Penalty: OOS annual shortfall (heavy weight = 3x)
    - Bonus: pass all criteria
    """
    dd_excess = max(0.0, abs(min(row.get("max_drawdown", 0.0), 0.0)) - args.target_max_drawdown)
    factor_penalty = 0.005 * max(int(row.get("factor_count", 1)) - 1, 0)
    oos_annual = row.get("oos_annual", 0.0)
    oos_shortfall = max(0.0, args.target_oos_annual - oos_annual)
    oos_penalty = 3.0 * oos_shortfall  # Heavy penalty for OOS failure
    pass_bonus = 1.0 if row.get("passed_full") else 0.0
    return float(
        row.get("annual_return", 0.0)
        - 2.0 * dd_excess
        - factor_penalty
        - oos_penalty
        + pass_bonus
...
def build_strategy(base: Strategy, tag: str, registry: FactorRegistry, topk: int, args) -> Strategy:
    ...
    return replace(
        base,
        name=f"alpha158_lgbm_biweek_wf__{tag}",
        display_name=f"alpha158_lgbm_biweek_wf_{tag}",
        description="Alpha158 LGBM walk-forward biweekly search candidate",
        registry=registry,
        weights={"alpha": 1.0},
        topk=topk,
...
def evaluate_candidate_wf(
    ...
    """Walk-forward evaluation of a single factor combination.

    Uses walk_forward_train_predict to generate OOS signals, then runs
    the formal Qlib backtest. OOS metrics are computed on walk-forward
    test periods only.
    """
    ...
    try:
        ...
        signal, oos_start, windows = walk_forward_train_predict(
            monthly_df,
            train_start=args.train_start,
            data_end=args.data_end,
            train_window_years=args.wf_train_years,
            test_window_years=args.wf_test_years,
        ...
        row["wf_oos_start"] = oos_start
        row["wf_windows"] = len(windows)
        # Count unique windows that produced models (heuristic: all train windows)
        row["wf_models"] = len(windows)

        if signal is None or signal.dropna().empty:
            row.update({"status": "empty_signal"})
            append_row(args.output, row)
            return row

        # Generate selections from walk-forward signal
        df_sel = extract_topk(
            signal,
            rebalance_dates,
            topk=topk,
        ...
        # Compute metrics
        # Full OOS: all dates from first test period onwards
        full_oos = period_metrics(bt.daily_returns, start=oos_start) if oos_start else {}
        # Train: all dates before first test period
        train = period_metrics(bt.daily_returns, end=oos_start) if oos_start else {}

        # Strict walk-forward OOS: only test period dates
        wf_oos_returns = []
        if windows:
            for w in windows:
                w_ret = bt.daily_returns[
                    (bt.daily_returns.index >= pd.Timestamp(w["test_start"]))
                    & (bt.daily_returns.index <= pd.Timestamp(w["test_end"]))
                ]
                if not w_ret.empty:
                    wf_oos_returns.append(w_ret)
        if wf_oos_returns:
            wf_oos = period_metrics(pd.concat(wf_oos_returns))
        else:
            wf_oos = {"annual": 0.0, "max_dd": 0.0, "sharpe": 0.0, "total": 0.0}

        # Check passing criteria (relaxed targets with OOS constraint)
        passed = bool(
            bt.annual_return >= args.target_annual
            and bt.max_drawdown >= -args.target_max_drawdown
            and full_oos.get("annual", 0.0) >= args.target_oos_annual
            and topk <= 10
        ...
        row.update(
            {
                ...
                "oos_annual": full_oos.get("annual", 0.0),
                "oos_max_dd": full_oos.get("max_dd", 0.0),
                "oos_sharpe": full_oos.get("sharpe", 0.0),
                "train_annual": train.get("annual", 0.0),
                "train_max_dd": train.get("max_dd", 0.0),
                "train_sharpe": train.get("sharpe", 0.0),
                "wf_oos_annual": wf_oos.get("annual", 0.0),
                "wf_oos_max_dd": wf_oos.get("max_dd", 0.0),
                "wf_oos_sharpe": wf_oos.get("sharpe", 0.0),
                "passed_full": passed,
                "results_file": bt.metadata.get("results_file", ""),
                "selection_file": str(selection_path),
            }
        )
        row["rank_score"] = rank_score_wf(row, args)
...
def search_wf(args) -> pd.DataFrame:
    ...
    print(f"[INFO] Walk-forward: {args.wf_train_years}yr train -> {args.wf_test_years}yr test, "
          f"step={args.wf_step_years}yr")
    print(f"[INFO] Targets: full>={args.target_annual:.0%}, OOS>={args.target_oos_annual:.0%}, "
          f"dd<{args.target_max_drawdown:.0%}")
        ...
        for combo in frontier[: args.max_expansions_per_level]:
            ...
            for topk in topk_options:
                ...
                if row.get("status") == "ok":
                    ...
                    print(
                        f"  {row['tag']} full={row['annual_return']:.2%} "
                        f"dd={row['max_drawdown']:.2%} oos={row['oos_annual']:.2%} "
                        f"wf_oos={row['wf_oos_annual']:.2%} rank={row['rank_score']:.3f}"
                    )
                else:
    ...
    if args.output.exists():
        ...
        if "rank_score" in final.columns:
            final = final.sort_values(
                ["passed_full", "rank_score"], ascending=[False, False]
            )
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Walk-forward search: Alpha158 LGBM biweekly top-k strategies."
    )
    # Data / factor inputs
    parser.add_argument("--scan-csv", default=str(PROJECT_ROOT / "results" / "alpha158_scan.csv"))
    parser.add_argument("--output", default=str(PROJECT_ROOT / "results" / "alpha158_lgbm_biweek_wf_search.csv"))
    ...
    parser.add_argument("--wf-test-years", type=int, default=1,
                        help="Test (OOS) window length in years")
    parser.add_argument("--wf-step-years", type=int, default=1,
                        help="Window step size in years")

    # Targets (relaxed for walk-forward)
    parser.add_argument("--target-annual", type=float, default=0.18,
                        help="Minimum full-sample annual return")
    parser.add_argument("--target-max-drawdown", type=float, default=0.10,
                        help="Maximum drawdown (absolute value)")
    parser.add_argument("--target-oos-annual", type=float, default=0.12,
                        help="Minimum OOS annual return (walk-forward)")
...
def main() -> int:
    ...
    final = search_wf(args)
    if final.empty:
        print("[WARN] No evaluations completed.")
        return 1

    ok = final[final["status"] == "ok"].copy() if "status" in final.columns else pd.DataFrame()
    passed = ok[ok["passed_full"].astype(bool)] if "passed_full" in ok.columns else pd.DataFrame()
    print("\n[SUMMARY] Walk-Forward Search")
    ...
    if ok.empty:
        ...
        existing_cols = [c for c in cols if c in show.columns]

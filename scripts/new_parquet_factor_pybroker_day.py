#!/usr/bin/env python3
"""
复核新增 parquet 日频正式单因子在 PyBroker 下的结果。

默认口径：
- 样本来源: results/new_parquet_factor_results.csv
- 过滤: status=success, annual_return > 0, sharpe_ratio > 0
- 排序: 先按 Qlib 夏普，再按 Qlib 年化
- 输出: results/new_parquet_factor_pybroker_day.csv / .md
"""
...
SOURCE_CSV = RESULTS_DIR / "new_parquet_factor_results.csv"
OUT_CSV = RESULTS_DIR / "new_parquet_factor_pybroker_day.csv"
OUT_MD = RESULTS_DIR / "new_parquet_factor_pybroker_day.md"
...
def _load_candidates(args: argparse.Namespace) -> pd.DataFrame:
    ...
    df = df[df["freq"] == "day"].copy()
    df = df[df["annual_return"] > args.min_annual]
    df = df[df["sharpe_ratio"] > args.min_sharpe]
    df = df.sort_values(["sharpe_ratio", "annual_return"], ascending=False).reset_index(drop=True)
...
def main() -> None:
    ...
    if out.empty:
        ...
        OUT_MD.write_text("# New Parquet Factor PyBroker Day\n\nNo rows.\n", encoding="utf-8")
        print(f"已保存: {OUT_CSV}")
        print(f"已保存: {OUT_MD}")
        return
    ...
    OUT_MD.write_text(
        "# New Parquet Factor PyBroker Day\n\n"
        f"Rows: {len(out)}\n\n"

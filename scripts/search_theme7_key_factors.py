...
ALPHA158_TECH = {"BETA60", "MA60", "ROC20", "RSV20", "CORD10", "WVMA10", "VSUMD10"}


THEME_PROFILES = {
    "full": {
        "cashflow": ["fcff_to_mv", "ocf_to_ev"],
        "earnings_growth": ["operate_profit_inc", "total_revenue_inc"],
        "long_trend": ["BETA60", "MA60"],
        # Qlib 里的 ROCd 更像“当前位置相对 d 日前的反转/超跌强弱”，优先保留它做短反转。
        "short_reversal": ["ROC20"],
        "bear_value": ["book_to_market", "ebit_to_mv"],
        "liquidity": ["current_ratio_fina"],
        "weekly_price_volume_growth": ["CORD10", "WVMA10"],
    },
    "focused": {
        "cashflow": ["fcff_to_mv", "ocf_to_ev"],
        "earnings_growth": ["operate_profit_inc", "total_revenue_inc"],
        "long_trend": ["BETA60", "MA60"],
        "short_reversal": ["ROC20"],
        "bear_value": ["ebit_to_mv"],
        "liquidity": ["current_ratio_fina"],
        "weekly_price_volume_growth": ["WVMA10"],
    },
}
...
def parse_args():
    ...
    parser.add_argument(
        "--search-name",
        default="theme7_key_factors_stage1_v1",
        help="结果目录名",
    )
    parser.add_argument(
        "--engine",
...
def candidate_rows(theme_candidates: dict[str, list[str]]):
    keys = list(theme_candidates.keys())
    values_list = [theme_candidates[key] for key in keys]
    for values in itertools.product(*values_list):
        row = dict(zip(keys, values))
        features = list(values)
        tag = "_".join(str(value).replace("_to_", "_").replace("_fina", "") for value in values)
...
def build_candidate_cfg(base_cfg: dict, search_name: str, tag: str, features: list[str], preset_name: str) -> dict:
    ...
    cfg["name"] = f"theme7_{tag}_{preset_name}"
...
def main():
    ...
    theme_candidates = THEME_PROFILES[str(args.profile)]
    candidates = list(candidate_rows(theme_candidates))
    if args.offset > 0 or args.stride > 1:
        candidates = candidates[int(args.offset) :: max(int(args.stride), 1)]
    if args.limit > 0:
        candidates = candidates[: int(args.limit)]

    rows = []
    for tag, theme_map, features in candidates:
            ...
            rows.append(
                {
                    ...
                    "cashflow": theme_map["cashflow"],
                    "earnings_growth": theme_map["earnings_growth"],
                    "long_trend": theme_map["long_trend"],
                    "short_reversal": theme_map["short_reversal"],
                    "bear_value": theme_map["bear_value"],
                    "liquidity": theme_map["liquidity"],
                    "weekly_price_volume_growth": theme_map["weekly_price_volume_growth"],
                    "valid_mean_rank_ic": float(metrics.get("valid_mean_rank_ic", 0.0)),
                    "full_annual_return": float(full_window["annual_return"]),
                    "full_max_drawdown": float(full_window["max_drawdown"]),
                    "full_sharpe_ratio": float(full_window["sharpe_ratio"]),

...
def build_overlay_config(overlay_cfg: dict) -> OverlayConfig:
    """从策略 YAML 的 overlay 段构造 OverlayConfig。

    回测（apply_overlay_to_backtest_result）与实盘仓位复刻（export_target.compute_live_exposure）
    共用此映射，保证两侧 overlay 参数完全一致。
    """
    overlay_cfg = dict(overlay_cfg or {})
